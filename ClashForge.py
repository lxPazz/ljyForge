...
CLASH_API_SECRET = ""
TIMEOUT = 2 # 延迟测试超时
SPEED_TIMEOUT = 10 # 单节点测速最长耗时(秒)
MAX_CONCURRENT_TESTS = 64 # 延迟测试并发
MAX_SPEED_TEST_COUNT = 50 # 选取延迟最低的前N个节点进行带宽测速
LIMIT = 20 # 最终保留前20个
CONFIG_FILE = 'clash_config.yaml'
INPUT = "input" 
BAN = ["中国", "China", "CN", "电信", "移动", "联通", "回国", "教育网"]
# ==============================================
...
clash_config_template = {
    ...
    "dns": {
        ...
        "fallback": ["https://doh.dns.sb/dns-query", "https://dns.cloudflare.com/dns-query", "tls://8.8.4.4:853"],
...
def parse_hysteria2_link(link):
    ...
    return {"name": f"{name}", "server": server, "port": port, "type": "hysteria2", "password": uuid, "auth": uuid, "sni": sni, "skip-cert-verify": not insecure, "client-fingerprint": "chrome"}

def parse_ss_link(link):
    link = link[5:]
    if "#" in link: config_part, name = link.split('#')
    else: config_part, name = link, ""
    ...
    return {"name": urllib.parse.unquote(name), "type": "ss", "server": server, "port": int(port), "cipher": cipher, "password": password, "udp": True}
...
def parse_trojan_link(link):
    ...
    return {"name": urllib.parse.unquote(name), "type": "trojan", "server": host, "port": int(port), "password": password, "sni": urllib.parse.parse_qs(query).get("sni", [""])[0], "skip-cert-verify": urllib.parse.parse_qs(query).get("skip-cert-verify", ["false"])[0] == "true"}
        ...
        if response.status_code == 200:
            ...
            if 'proxies:' in content:
                ...
                try:
                    ...
                    decoded_content = urllib.parse.unquote(decoded_bytes.decode('utf-8'))
...
def parse_proxy_link(link):
    if link.startswith(("hysteria2://", "hy2://")): return parse_hysteria2_link(link)
    ...
    for file_path in yaml_files:
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                if config and 'proxies' in config: load_nodes.extend(config['proxies'])
        except: pass
    return load_nodes
...
def handle_links(new_links, resolve_name_conflicts):
    for new_link in new_links:
        if new_link.startswith(("hysteria2://", "hy2://", "trojan://", "ss://", "vless://", "vmess://")):
            ...
            if node: resolve_name_conflicts(node)
...
def generate_clash_config(links, load_nodes):
    ...
    def resolve_name_conflicts(node):
        ...
        if not_contains(name):
            if name in existing_names: name = add_random_suffix(name, existing_names)
            existing_names.add(name)
            node["name"] = name
            final_nodes.append(node)

    for node in load_nodes: resolve_name_conflicts(node)
        ...
        try:
            if link.startswith(("hysteria2://", "hy2://", "trojan://", "ss://", "vless://", "vmess://")):
                ...
                if '|ss' in link:
                    ...
                    for node in new_links: resolve_name_conflicts(node)
                if '{' in link:
    ...
    if config["proxies"]:
        ...
        print(f"生成配置文件 {CONFIG_FILE}，包含 {len(final_nodes)} 个节点")
    else:
        print('无有效节点')
...
class ProxyTestResult:
    def __init__(self, name: str, delay: Optional[float] = None, speed: float = 0.0):
        self.name = name
        self.delay = delay if delay is not None else float('inf')
        self.speed = speed # MB/s
        self.status = "ok" if delay is not None else "fail"
    @property
    def is_valid(self) -> bool: return self.status == "ok"

def test_proxy_speed_serial(proxy_name):
    # 切换节点
    url = f"http://{CLASH_API_HOST}:{CLASH_API_PORTS[0]}/proxies/节点选择"
    try:
        requests.put(url, json={"name": proxy_name}, timeout=2)
    except:
        return 0.0
    
    proxies = {'http': f'http://127.0.0.1:{CLASH_PROXY_PORT}', 'https': f'http://127.0.0.1:{CLASH_PROXY_PORT}'}
    try:
        start_time = time.time()
        with requests.get(SPEED_TEST_URL, proxies=proxies, stream=True, timeout=5, verify=False) as r:
            r.raise_for_status()
            total_bytes = 0
            for chunk in r.iter_content(chunk_size=65536):
                if chunk: total_bytes += len(chunk)
                if time.time() - start_time > SPEED_TIMEOUT: break
...
class ClashAPI:
    def __init__(self, host, ports, secret=""):
    ...
    async def check_connection(self):
        for port in self.ports:
            try:
                resp = await self.client.get(f"http://{self.host}:{port}/version")
                if resp.status_code == 200:
                    self.base_url = f"http://{self.host}:{port}"
                    return True
            except: continue
        return False
    async def test_proxy_delay(self, proxy_name):
        async with self.semaphore:
            try:
                resp = await self.client.get(f"{self.base_url}/proxies/{proxy_name}/delay", headers=self.headers, params={"url": TEST_URL, "timeout": int(TIMEOUT*1000)})
                if resp.status_code == 200: return ProxyTestResult(proxy_name, resp.json().get("delay"))
            except: pass
            return ProxyTestResult(proxy_name)

async def test_group_proxies(clash_api, proxies):
    print(f"开始延迟筛选 {len(proxies)} 个节点...")
    tasks = [clash_api.test_proxy_delay(p) for p in proxies]
    results = []
    for f in asyncio.as_completed(tasks):
        results.append(await f)
        print(f"\r进度: {len(results)}/{len(tasks)}", end="", flush=True)
    print()
    return results
...
async def proxy_clean():
    ...
    print(f"========== 开始节点测速 (目标前{LIMIT}名) ==========")
    
    with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
        config = json.load(f)
    if "proxies" not in config: return

    all_proxies = [p['name'] for p in config['proxies']]
    if not all_proxies: return

    async with ClashAPI(CLASH_API_HOST, CLASH_API_PORTS, CLASH_API_SECRET) as api:
        if not await api.check_connection():
            print("Clash API 未运行")
            return
        delay_results = await test_group_proxies(api, all_proxies)
    
    alive = [r for r in delay_results if r.is_valid]
    alive.sort(key=lambda x: x.delay)
    candidates = alive[:MAX_SPEED_TEST_COUNT]
    print(f"有效节点 {len(alive)} 个，取延迟最低的 {len(candidates)} 个进行带宽测试...")

    speed_results = []
    for i, node in enumerate(candidates):
        print(f"[{i+1}/{len(candidates)}] 测试 {node.name} ... ", end="")
        sys.stdout.flush()
        speed = test_proxy_speed_serial(node.name)
        node.speed = speed
        speed_results.append(node)
        print(f"{speed:.2f} MB/s")

    speed_results.sort(key=lambda x: x.speed, reverse=True)
    final = speed_results[:LIMIT]
    final_names = [r.name for r in final]

    print(f"\n========== 最终保留 {len(final)} 个节点 ==========")
    rename_map = {}
    for r in final:
        new_name = f"{r.name} [{r.speed:.1f} MB/s]"
        rename_map[r.name] = new_name
        print(f"{new_name} | 延迟: {r.delay}ms")
    ...
    # 按速度排序重组 proxies 列表
    new_proxies.sort(key=lambda x: float(re.search(r'\[([\d\.]+) MB/s\]', x['name']).group(1)), reverse=True)
    config["proxies"] = new_proxies

    sorted_names = [p['name'] for p in new_proxies]
    for group in config["proxy-groups"]:
        if group["name"] in ["节点选择", "故障转移", "自动选择", "手动选择"]:
            group["proxies"] = sorted_names
...
def ensure_executable(file_path):
    if platform.system().lower() in ['linux', 'darwin']: os.chmod(file_path, 0o755)
...
def download_clash():
    ...
    try:
        ...
        for asset in assets:
            name = asset.get("name", "")
            if os_type in target_map and target_map[os_type] in name:
                 if (os_type == "windows" and name.endswith('.zip')) or name.endswith('.gz'):
                    download_url = asset["browser_download_url"]
                    break
        
        if download_url:
            print(f"Downloading mihomo core...")
            ...
            if filename.endswith('.zip'):
                with zipfile.ZipFile(filename, 'r') as z:
                    z.extractall()
                    for n in z.namelist():
                        if os.path.exists(n): os.rename(n, new_name); break
            elif filename.endswith('.gz'):
                with gzip.open(filename, 'rb') as f_in, open(new_name, 'wb') as f_out:
...
def start_clash():
    ...
    while True:
        ...
        output_lines = []
        threading.Thread(target=read_output, args=(clash_process.stdout, output_lines), daemon=True).start()
        
        start_time = time.time()
        while time.time() - start_time < 5:
            time.sleep(1)
            try:
                if requests.get(f"http://{CLASH_API_HOST}:{CLASH_API_PORTS[0]}/version", timeout=1).status_code == 200:
                    return clash_process
            except: pass
            
            if output_lines and "Parse config error" in output_lines[-1]:
...
def parse_template(template_url, datetime_vars):
    def replace_template(match):
        ...
        if template_content == 'x': return '{x}'
        result = ''
        current_char = ''
        for char in template_content:
            if char in datetime_vars:
                if current_char: result += current_char; current_char = ''
                result += datetime_vars[char]
            else:
                current_char += char
        if current_char: result += current_char
        return result
...
def work(links, check=False, allowed_types=[], only_check=False):
    try:
        ...
        if check or only_check:
            clash_process = None
            try:
                print(f"=================== 启动Clash核心进行测速 ======================")
                clash_process = start_clash()
                
                # 初始化：切换到直连防止死循环
                url = f"http://{CLASH_API_HOST}:{CLASH_API_PORTS[0]}/proxies/节点选择"
                try: requests.put(url, json={"name": "DIRECT"}, timeout=2)
                except: pass
...
if __name__ == '__main__':
    ...
    # check=True: 开启测速筛选
    # only_check=False: 先下载订阅再测速；如果为True则直接读取本地已有config文件进行测速
    # allowed_types: 需要保留的节点类型
    work(
        links, 
        check=True, 
        only_check=False, 
